...
export const oscilloscopeNode: NodeImplementation = {
  ...
  assembly: {
    ...
    emit(planNode, helpers) {
      ...
      bodyLines.push("  let scopeTime: f32 = unchecked(scopeMonitorTime[monitorIndex]);");
      bodyLines.push("  let targetSamples: i32 = unchecked(scopeMonitorTargetSamples[monitorIndex]);");
      bodyLines.push("  let refreshCounter: i32 = unchecked(scopeMonitorRefreshCounter[monitorIndex]);");
      bodyLines.push("  let refreshDue: bool = refreshCounter <= 0 || targetSamples <= 0;");
      bodyLines.push("  if (refreshDue) {");
      bodyLines.push("    scopeScale = " + scaleExpr + ";");
      bodyLines.push(`    if (scopeScale < ${MIN_SCALE}) scopeScale = ${MIN_SCALE};`);
      bodyLines.push(`    if (scopeScale > ${MAX_SCALE}) scopeScale = ${MAX_SCALE};`);
      bodyLines.push("    scopeTime = " + timeExpr + ";");
      bodyLines.push(`    if (scopeTime < ${MIN_TIME}) scopeTime = ${MIN_TIME};`);
      bodyLines.push(`    if (scopeTime > ${MAX_TIME}) scopeTime = ${MAX_TIME};`);
      bodyLines.push("    targetSamples = Mathf.round(scopeTime * SAMPLE_RATE) as i32;");
      ...
      bodyLines.push("    refreshCounter = SCOPE_REFRESH_INTERVAL;");
      bodyLines.push("    unchecked(scopeMonitorScale[monitorIndex] = scopeScale);");
      bodyLines.push("    unchecked(scopeMonitorTime[monitorIndex] = scopeTime);");
      bodyLines.push("    unchecked(scopeMonitorTargetSamples[monitorIndex] = targetSamples);");
      bodyLines.push("  } else {");
      bodyLines.push("    if (targetSamples < 1) targetSamples = 1;");
      bodyLines.push("  }");
      bodyLines.push("  if (refreshCounter > 0) {");
      bodyLines.push("    refreshCounter -= 1;");
      bodyLines.push("  }");
      bodyLines.push("  unchecked(scopeMonitorRefreshCounter[monitorIndex] = refreshCounter);");
